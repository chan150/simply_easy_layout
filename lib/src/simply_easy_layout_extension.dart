...
extension EasyLayoutExtension on BuildContext {
  ...
  SimplyEasyLayout get layout {
    final easyLayout = SimplyEasyLayout(
      ...
      axis: SimplyEasyLayoutFactory().preferredAxis,
      screenWidth: w,
      screenHeight: h,
    );
...
extension EasyLayoutTupleExtension on (num, num) {
  double get width => this.$1.toDouble();

  double get height => this.$2.toDouble();
  ...
  Widget cover({Key? key, Widget? child, BoxDecoration? decoration, AlignmentGeometry? alignment}) {
    return Container(
      key: key,
